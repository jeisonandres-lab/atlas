...
class reportesController extends FPDF
{
    ...
    public function generarPDFdependencia(string $dependencia)
    {
        ...
        $this->SetFillColor(25, 41, 187);
        $this->SetTextColor(255, 255, 255);
        $datosEmpleado = $this->personalData->getDatosEmpleadoFiltro('depe.idDependencia = ?', [$dependencia]);
        $i = 0;
        ...
        )
    {
        ...
        if (!empty($edad)) {
            $conditions[] = 'dp.edadPersonal = ?';
            $params[] = $edad;
        }
        if (!empty($civil)) {
            $conditions[] = 'dp.estadoCivil = ?';
            $params[] = $civil;
        }
        if (!empty($vivienda)) {
            $conditions[] = 'ubi.vivienda = ?';
            $params[] = $vivienda;
        }
        if (!empty($idEstatus)) {
        ...
        if (!empty($nivelAcademico)) {
            $conditions[] = 'de.nivelAcademico = ?';
            $params[] = $nivelAcademico;
        }
        if (!empty($idDependencia)) {
            $conditions[] = 'de.idDependencia = ?';
            $params[] = $idDependencia;
        }
        if (!empty($idDepartamento)) {
            $conditions[] = 'de.idDepartamento = ?';
            $params[] = $idDepartamento;
        }
        if (!empty($idestado)) {
            $conditions[] = 'ubi.idEstado = ?';
            $params[] = $idestado;
        }
        if (!empty($idMunicipio)) {
            $conditions[] = 'ubi.idMunicipio = ?';
            $params[] = $idMunicipio;
        }
        if (!empty($idParroquia)) {
            $conditions[] = 'ubi.idParroquia = ?';
            $params[] = $idParroquia;
        }
        if (!empty($fecha_Ini) && !empty($fecha_Fin)) {
            $conditions[] = 'de.fechaING BETWEEN ? AND ?';
            $params[] = $fecha_Ini;
            $params[] = $fecha_Fin;
        }
        ...
        if (empty($where)) {
            $where = '1=1'; // Si no hay condiciones, seleccionar todo
        }

        $datosEmpleado = $this->personalData->getDatosPersonalizadoReporte($where, $params);
        $i = 0;
        ...
        if (is_array($datosEmpleado)) {
            ...
            foreach ($datosEmpleado as $dato) {
                if (is_array($dato)) {
                    $registroAuditoria = $this->auditoriaController->registrarAuditoria($this->idUsuario, 'Creaste un pdf de formato personalizado', 'El usuario ' . $this->nombreUsuario . ' ha descargado un pdf de los empleados en tamaño carta formato vertical');
...
//  $datosEmpleado = $this->personalData->getDatosPersonalizadoReporte(
//             'c.id_cargo = ? AND
//             dp.sexo = ? AND
//             dp.edadPersonal = ? AND
//             dp.estadoCivil = ? AND
//             ubi.vivienda = ? AND
//             e.id_estatus = ? AND
//             de.nivelAcademico = ? AND
//             depe.idDependencia = ? AND
//             de.idDepartamento = ? AND
//             ubi.idEstado = ? AND
//             ubi.idMunicipio  = ? AND
//             ubi.idParroquia  = ?
//             de.fechaING BETWEEN ? AND ?',
//             [
//                 $idCargo,
//                 $sexo,
//                 $edad,
