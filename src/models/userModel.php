...
class userModel extends Conexion
{
    ...
    private function existeUsuario(string $user)
    {
        $sql = $this->ejecutarConsulta("SELECT
         us.id_user,
         us.idEmpleado as idEmpleado,
         dtp.cedula as cedula,
         ...
         r.rol as rol
         FROM users us INNER JOIN datosempleados dte ON us.idEmpleado = dte.id_empleados
         INNER JOIN datospersonales dtp ON dte.idPersonal = dtp.id_personal
         INNER JOIN rol r ON us.idRol = r.id_rol WHERE us.nameUser = '$user'");
        return $sql;
    }
